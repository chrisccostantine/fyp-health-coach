...
def _normalized_medical_review_flags(profile: UserProfile):
    diet_data = profile.diet if isinstance(profile.diet, dict) else {}
    raw_flags = []
    for key in ("conditions", "medical_conditions", "medications", "contraindications"):
        value = diet_data.get(key)
        if isinstance(value, list):
            raw_flags.extend(value)
        elif value:
...
def _assess_plan_safety(profile: UserProfile, goal: Goal):
    ...
    medical_flags = _normalized_medical_review_flags(profile)
    ...
    if age is None or height_cm is None or weight_kg is None:
        ...
        if age < 16:
            ...
            review_reasons.append("Users aged 16-17 require dietitian review before using a personalized plan.")
    ...
    if matched_flags:
        review_reasons.append("High-risk medical factors were entered: " + ", ".join(matched_flags) + ".")
    elif medical_flags:
        review_reasons.append("Medical conditions or medications were entered and require dietitian review.")

    if review_reasons:
        warnings.append("This plan must be approved by your dietitian before use.")
...
def _plan_review_payload(session: dict | None, safety: dict | None = None):
    safety = safety or {}
    safety_requires_review = bool(safety.get("requires_review"))
    if not session or (session.get("role") or "user") != "user":
        return None
    manager_user_id = session.get("managed_by_user_id")
    if not manager_user_id and not safety_requires_review:
        return None
    reasons = safety.get("review_reasons") or []
    note = " ".join(reasons) if reasons else "Plan generated by managed client and queued for dietitian review."
    return {
        "required": True,
...
def plan_today():
    ...
    if safety.get("requires_review") and not (session and session.get("managed_by_user_id")):
        return jsonify(
            {
                "error": "This profile requires dietitian review before a plan can be generated.",
                "safety": safety,
            }
        ), 422
    ...
    if regeneration_scope in {"meals", "workouts", "day"}:
        ...
        if generation_warnings:
            plan_payload["generation_warnings"] = generation_warnings
    review_payload = _plan_review_payload(session, safety)
    if review_payload:
